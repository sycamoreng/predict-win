...
const props = defineProps<{
  ...
  url?: string
  title?: string
  imageCardProps?: Record<string, any>
}>()
...
const shareMode = ref<'text' | 'image'>(props.imageCardProps ? 'image' : 'text')
...
const doNativeShareImage = async () => {
  ...
  try {
    ...
    if (!blob) return
    const file = new File([blob], 'sycamore-predictor.png', { type: 'image/png' })
    ...
    <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="$emit('close')">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-fade-up max-h-[90vh] overflow-y-auto">
        <div class="p-6 space-y-5">
          ...
          <!-- Mode toggle (only when image card is available) -->
          <div v-if="imageCardProps" class="flex rounded-xl bg-ink-100 p-1 gap-1">
            <button
          ...
          <!-- Image preview -->
          <div v-if="shareMode === 'image' && imageCardProps" class="space-y-4">
            <div class="rounded-xl overflow-hidden border border-ink-100 shadow-soft">
              <div class="transform scale-[0.6] origin-top-left h-[324px] w-full relative">
                <div ref="imageCardRef" class="absolute top-0 left-0">
                  <ShareImageCard v-bind="imageCardProps" />
                </div>
              </div>
            </div>
