...
const { canNativeShare, canNativeShareFiles, nativeShare, nativeShareWithImage, generateImage, downloadImage, shareToTwitter, shareToWhatsApp, shareToThreads, shareToInstagramStory, copyToClipboard, siteUrl } = useShare()
const { trackPulseEvent } = useAuth()
const copied = ref(false)
const generatingImage = ref(false)
const imageGenerated = ref(false)
const shareMode = ref<'text' | 'image'>(props.imageData ? 'image' : 'text')
...
const doNativeShareImage = async () => {
  if (!imageCardRef.value) return
  generatingImage.value = true
  try {
    const blob = await generateImage(imageCardRef.value)
    if (!blob) return
    const file = new File([blob], 'sycamore-predictor-stats.png', { type: 'image/png' })
    trackPulseEvent('shared', { method: 'native-image', title: props.title })
    const shared = await nativeShareWithImage(file, { text: props.text, title: props.title })
...
const doDownloadImage = async () => {
  ...
  try {
    ...
    trackPulseEvent('shared', { method: 'download-image', title: props.title })
    setTimeout(() => (imageGenerated.value = false), 3000)
  } finally {
    generatingImage.value = false
  }
}
...
const platforms = [
  ...
  { id: 'instagram-story', label: 'IG Story', color: 'bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#F77737]' },
]
...
const shareTo = (platform: string) => {
  ...
  else if (platform === 'instagram-story') shareToInstagramStory(opts)
  emit('close')
}
</script>
...
<template>
  <Teleport to="body">
    <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="$emit('close')">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-fade-up max-h-[90vh] overflow-y-auto">
        <div class="p-6 space-y-5">
          ...
          <!-- Mode toggle (only when image data available) -->
          <div v-if="imageData" class="flex rounded-xl bg-ink-100 p-1 gap-1">
            <button
              @click="shareMode = 'image'"
              :class="[shareMode === 'image' ? 'bg-white shadow-sm text-ink-900' : 'text-ink-500 hover:text-ink-700', 'flex-1 py-2 px-3 rounded-lg text-sm font-semibold transition-all']"
            >
              Image
            </button>
            <button
              @click="shareMode = 'text'"
              :class="[shareMode === 'text' ? 'bg-white shadow-sm text-ink-900' : 'text-ink-500 hover:text-ink-700', 'flex-1 py-2 px-3 rounded-lg text-sm font-semibold transition-all']"
            >
              Text
            </button>
          </div>
          ...
          <!-- Image preview -->
          <div v-if="shareMode === 'image' && imageData" class="space-y-4">
            <div class="rounded-xl overflow-hidden border border-ink-100 shadow-soft">
              <div class="transform scale-[0.6] origin-top-left h-[324px] w-full relative">
                <div ref="imageCardRef" class="absolute top-0 left-0">
                  <ShareImageCard
                    :username="imageData.username"
                    :total-points="imageData.totalPoints"
                    :predictions-count="imageData.predictionsCount"
                    :correct-predictions="imageData.correctPredictions"
                    :exact-scorelines="imageData.exactScorelines"
                    :rank="imageData.rank"
                    :backed-team="imageData.backedTeam"
                  />
                </div>
              </div>
            </div>
            ...
            <!-- Share image actions -->
            <button
              v-if="canNativeShareFiles"
              @click="doNativeShareImage"
              :disabled="generatingImage"
              class="w-full btn-primary py-3 text-sm flex items-center justify-center gap-2 disabled:opacity-50"
            >
              <svg v-if="!generatingImage" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
            ...
            <button
              @click="doDownloadImage"
              :disabled="generatingImage"
              class="w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-ink-100 text-sm font-semibold text-ink-700 hover:bg-ink-50 transition disabled:opacity-50"
            >
              <svg v-if="!generatingImage && !imageGenerated" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
              <svg v-else-if="imageGenerated" class="w-4 h-4 text-mint-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
              <svg v-else class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
              {{ imageGenerated ? 'Saved!' : generatingImage ? 'Generating...' : 'Download image' }}
            </button>

            <p class="text-xs text-ink-400 text-center">Download and post to your story or feed</p>
          </div>

          <!-- Text mode preview -->
          <template v-if="shareMode === 'text'">
            ...
            <div class="grid grid-cols-4 gap-3">
              ...
              >
                <div :class="[p.color, 'w-12 h-12 rounded-full grid place-items-center']">
                  ...
                  <!-- Instagram Story -->
                  <svg v-if="p.id === 'instagram-story'" class="w-6 h-6 text-white" viewBox="0 0 24 24" fill="currentColor">
