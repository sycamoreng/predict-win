...
const loadGroupMembers = async (group: Group) => {
  ...
  const { data: members } = await supabase
    .from('group_members')
    .select('id, user_id, role, user:synced_users!group_members_user_id_fkey(id, name, username, email)')
...
const displayMemberName = (m: GroupMember) => {
  ...
  return m.user?.email?.split('@')[0] || 'Unknown'
}
...
<template>
  <div class="space-y-6 max-w-4xl mx-auto">
    ...
    <!-- Empty state -->
    <div v-else-if="myGroups.length === 0 && !selectedGroup" class="card p-12 sm:p-16 text-center animate-fade-up">
