...
-- 1) Use campaign_config week_start_date anchor (same as get_week_bounds) instead of date_trunc
-- 2) Partition RANK by is_staff so filtering doesn't create gaps
CREATE OR REPLACE VIEW weekly_leaderboard AS
WITH campaign AS (
  SELECT COALESCE(
    (SELECT week_start_date FROM campaign_config WHERE id = 1),
    '2026-06-11'::date
  ) AS anchor
),
current_week AS (
  SELECT
    (c.anchor + (FLOOR(((now() AT TIME ZONE 'UTC')::date - c.anchor) / 7.0)::int * 7) * interval '1 day')::timestamptz AS week_start,
    (c.anchor + (FLOOR(((now() AT TIME ZONE 'UTC')::date - c.anchor) / 7.0)::int * 7) * interval '1 day' + interval '7 days')::timestamptz AS week_end
  FROM campaign c
),
week_matches AS (
  SELECT m.id
