-- The weekly_leaderboard view is readable with the public anon key so the
-- leaderboard works for signed-out visitors. It exposed every participant's
-- email address, which is personal data that should not be public. The app
-- identifies the current user by id and never displays the email from this
-- view, so we drop the email column. (Account number and phone were never in
-- this view.) A DROP + CREATE is required because a view column cannot be
-- removed with CREATE OR REPLACE.
